...
groups the moduli that trace the same figure as each other.`,
  ...
  pisano sweep --max 40 --table`,
	...
	cmd.RunE = func(_ *cobra.Command, _ []string) error {
		...
		if table {
			fmt.Print(pisano.Table(rows))
			fmt.Println()
		}
		fmt.Println("zeros vs symmetry of the circular design")
		fmt.Println("----------------------------------------")
		fmt.Print(pisano.ZeroSymmetry(rows))
		fmt.Println()
		fmt.Println("turtle path shape")
		fmt.Println("-----------------")
		fmt.Print(pisano.TurtleShapes(rows))
		if dupes {
			fmt.Println()
			fmt.Println("turtle paths shared between moduli")
			fmt.Println("----------------------------------")
			fmt.Print(pisano.Duplicates(rows))
		}
		return nil
	}
	return cmd
