...
func buildGallery(h *Host, stdout, stderr io.Writer, out, svgDir string, cell int, theme pisano.Theme) error {
	...
	// The lead says what a Pisano period IS, because the page is titled for one
	// and then never defines it: a reader who arrived having searched those
	// words got 332 figures and no way in. The link used to point at term/,
	// which has been a redirect back to the site root since the terminal became
	// the site, so it sent everyone on a round trip to where they already were.
	err = pisano.WriteHTMLPage(f, "Pisano periods, drawn",
		`Take the Fibonacci numbers modulo <em>m</em> — keep only the remainder `+
			`after dividing by <em>m</em> — and the sequence repeats. The length of `+
			`that repeat is the <strong>Pisano period</strong>, written `+
			`π(<em>m</em>), and it is different for every modulus in a way the `+
			`arithmetic does not advertise: π(10) is 60, π(11) is 10. Draw the `+
			`residues instead of listing them and the repeat becomes a figure. `+
			`Every one here is generated by <code>pisano gallery</code>; for the `+
			`interactive version — the same designs drawn live in a shell compiled `+
			`to WebAssembly — see <a href="../">the desktop</a>.`,
		secs)
	closeFn()
	if err != nil {
		return err
